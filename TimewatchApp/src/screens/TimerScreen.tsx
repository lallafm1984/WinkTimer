...
const TIMER_MASCOT_VISUAL_SCALE = 1.28;
const TIMER_MASCOT_MAX_SCALE = 1.42;
...
export function getTimerLayoutMetrics(layoutScale: number) {
  return {
    mascotScale: Math.min(
      layoutScale * TIMER_MASCOT_VISUAL_SCALE,
      TIMER_MASCOT_MAX_SCALE,
    ),
    timerDisplayScale: layoutScale,
    topStripMinHeight: getScaledValue(TIMER_TOP_STRIP_MIN_HEIGHT, layoutScale),
  };
}
...
function isWinkJudgmentUnavailable(timer: TimerState, mode: TimerModePreset) {
  return (
    mode.id === 'winkControl' &&
...
function isSmileJudgmentUnavailable(timer: TimerState, mode: TimerModePreset) {
  return (
    mode.id === 'smileMode' &&
...
function getModeMenuDescription(
  mode: TimerModePreset,
  locale: Parameters<typeof createTranslator>[0],
) {
  return getLocalizedTimerModeMenuDescription(locale, mode.id);
}
...
function getHistoryTypeLabel(
  ...
  const eventLabelKeys: Record<
    Exclude<SessionHistoryEvent['type'], 'LAP'>,
    TranslationKey
  > = {
    END: 'event.END',
    RESET: 'event.RESET',
...
function SessionHistoryOverlay({
  ...
  return (
    <View style={styles.historyOverlay} testID="session-history-overlay">
      <View style={styles.historyHeader} testID="session-history-header">
        <View style={styles.historyTitleGroup}>
          <Text style={styles.historyTitle} testID="session-history-title">
            {t('timer.timeline')}
          </Text>
          <Pressable
...
export function TimerScreen() {
  ...
  const modeMenuLayoutMetrics = getModeMenuLayoutMetrics(responsiveLayoutScale);
  ...
  const [gestureUnavailableHintVisible, setGestureUnavailableHintVisible] =
  ...
  const winkReadyHintShown =
    shouldRenderWinkJudgmentHint && !gestureUnavailableHintVisible;
  const winkJudgmentHintShown = winkUnavailableHintShown || winkReadyHintShown;
  ...
  const canChangeMode =
    !isTimerAlertActive &&
    (timer.phase === 'idle' || timer.phase === 'ended' || stoppedState);
  const canReset = canResetTimer(timer, selectedMode) && !isFinishingSession;
  const shouldShowLapAction =
    timer.phase === 'active' &&
    selectedMode.lapGesture !== undefined &&
    !stoppedState;
  const canRecordLap = shouldShowLapAction && !isFinishingSession;
  ...
  const appTitle =
    visibleTimekeepingMode === 'timer'
      ? t('timer.timer')
      : t('timer.stopwatch');
  const primaryAction = stoppedState
    ? {
    ...
    : {
        label: timer.phase === 'ended' ? t('timer.restart') : t('common.start'),
  ...
  }, [canOpenTimerTargetPopup, showsTimerTargetControls, timerTargetPopupOpen]);
  ...
  React.useEffect(() => {
    ...
    setTimerTargetDraftDurationMs(effectiveTimerTargetDurationMs);
    setTimerTargetPopupOpen(true);
    consumeTimerTargetPopupRequest(timerTargetPopupRequestId);
  }, [
    consumeTimerTargetPopupRequest,
    effectiveTimerTargetDurationMs,
    setTimerTargetDraftDurationMs,
    ...
    <View style={styles.container}>
      ...
      {modeMenuVisible ? (
                  ...
                  <Pressable
                    ...
                    <View style={styles.modeMenuCopy}>
                      <View style={styles.modeMenuTitleRow}>
                        <View style={styles.modeMenuTitleCopy}>
                          <Text
                            style={[
                              styles.modeMenuTitle,
                              scaledLayoutActive && {
                                fontSize:
                                  modeMenuLayoutMetrics.titleFontSize,
                                lineHeight:
                                  modeMenuLayoutMetrics.titleLineHeight,
                              },
                            ]}>
                            {modeTitle}
                          </Text>
                          {modeDescription ? (
                            <Text
                              style={[
                                styles.modeMenuDescription,
                                scaledLayoutActive && {
                                  fontSize:
                                    modeMenuLayoutMetrics.summaryFontSize,
                                  lineHeight:
                      ...
                      <Text
                        style={[
                          ...
                          scaledLayoutActive && {
                            ...
                            lineHeight: modeMenuLayoutMetrics.summaryLineHeight,
                          },
                        ]}>
        ...
        <ArcadePanel style={styles.timerPanel}>
          ...
          <View style={styles.timerContentArea}>
            ...
            <View style={styles.actionDock} testID="timer-action-dock">
              <TimerActionButton
                label={shouldShowLapAction ? lapActionLabel : t('timer.reset')}
                gesture={shouldShowLapAction ? lapGesture : resetGesture}
                onPress={
                  shouldShowLapAction ? recordLapSession : resetTimerSession
                }
              ...
              <TimerActionButton
                ...
                disabled={false}
                accessibilityState={{
                  selected: !isTimerAlertActive && historyOpen,
                }}
              />
            </View>
          </View>
          ...
          {timerTargetPopupOpen ? (
            <View style={styles.timerTargetPopupBackdrop}>
              <View style={styles.timerTargetPopup} testID="timer-target-popup">
                        ...
                        return (
                          <Pressable
                            ...
                            style={({pressed}) => [
                              ...
                              selected &&
                                styles.timerTargetRecentButtonSelected,
                              pressed && styles.pressedControl,
                            ]}
      ...
      <View style={styles.modeSection} testID="mode-selector-bottom">
        ...
        <TimekeepingModeBar
          ...
          onSelectStopwatch={() => handleSelectTimekeepingMode('stopwatch')}
          onSelectTimer={() => handleSelectTimekeepingMode('timer')}
